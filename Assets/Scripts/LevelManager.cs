...
public class LevelManager : MonoBehaviour {
	...
	public void flipTheSwitch() {
		...
		if (Input.GetKeyDown (KeyCode.Space) && controlSwitch == true && switchOn == false) {

			theSwitch.turnOn ();
			switchOn = true; 
			if (CantMove == false) {
				vase.AddForce (force, ForceMode2D.Impulse);
			}
		}

		else if (Input.GetKeyDown (KeyCode.Space) && controlSwitch == true && switchOn == true) {
